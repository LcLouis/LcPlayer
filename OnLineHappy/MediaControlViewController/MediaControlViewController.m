...
- (void)viewDidLoad {
    [super viewDidLoad];
    // **** player delegate
    AMZPlayer *player = [(VideoViewController *)_delegate player];
...
{
    AMZPlayer *player = [(VideoViewController *)_delegate player];
    _isAMZPlayerPling = player.isPlaying;
...
- (void)seekCompletedWithPosition:(CGFloat)position {
    ...
    if (_isCompleted == YES) {
        ...
        AMZPlayer *player = [(VideoViewController *)_delegate player];
        if (_isAMZPlayerPling) {
            [player play];
        }else{
            _isAMZPlayerPling = NO;
            [player pause];
        }
        _isActive = YES;
        [self refreshControl];
...
#pragma mark - AMZMediaPlayerDelegate

- (void)mediaPlayerStateChanged:(AMZPlayerState)PlayerState {
    
    if (PlayerState != AMZPlayerStateError) {
        [self removeError];
    }
    AMZPlayer *player = [(VideoViewController *)_delegate player];
    if (PlayerState == AMZPlayerStateInitialized) {
        _isPrepared = NO;
        [self showLoading];
    }else if (PlayerState == AMZPlayerStatePrepared){
...
- (void)mediaPlayerBuffing:(AMZBufferingState)bufferingState {
    if (bufferingState == AMZPlayerBufferingStart) {
        [self showLoading];
    }
    else {
        [self performSelectorOnMainThread:@selector(removeLoading) withObject:nil waitUntilDone:NO];
...
- (void)showError {
    if (_errorView == nil) {
        ...
        indicatorLabel.text = @"主播不在，换个房间试试";
